  ...
  if (GetIpTos ())
    {
      ...
      if (m_tcb->m_ecnState != TcpSocketState::ECN_DISABLED && !CheckNoEcn (GetIpTos ()))
        {
          // Classic traffic have ECT(0) flags whereas L4S (Low Loss Low Latency Scalable Throughput) have ECT(1) flags set with the last received ipTos
  ...
  else
    {
      if (m_tcb->m_ecnState != TcpSocketState::ECN_DISABLED && p->GetSize () > 0)
        {
          // Classic traffic have ECT0 flags whereas L4S (Low Loss Low Latency Scalable Throughput) have ECT1 flags set
          SocketIpTosTag ipTosTag;
          if (m_congestionControl->GetName () == "TcpDctcp")
            {
              ipTosTag.SetTos (MarkEcnEct1 (GetIpTos ()));
...
TcpSocketBase::PersistTimeout ()
{
  ...
  if (m_tcb->m_ecnState != TcpSocketState::ECN_DISABLED)
    {
      SocketIpTosTag ipTosTag;
      // Classic traffic have ECT(0) flags whereas L4S (Low Loss Low Latency Scalable throughput) have ECT(1) flags set
      if (m_congestionControl->GetName () == "TcpDctcp")
        {
